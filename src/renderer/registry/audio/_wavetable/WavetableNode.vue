...
import { ChevronDown, ChevronRight, Waves } from 'lucide-vue-next'
...
import type { WaveformData } from '@/components/controls/WaveformEditor.vue'
...
const definition = computed<NodeDefinition | null>(() => {
  const nodeType = (props.data?.nodeType as string) ?? 'wavetable'
...
// Waveform data
const waveformData = computed<WaveformData>({
  get: () => {
    const preset = (props.data?.preset as string) ?? 'sine'
    const samples = (props.data?.waveform as number[]) ?? generateDefaultSamples(preset)
    return { samples, preset: preset as WaveformData['preset'] }
  },
  set: (value: WaveformData) => {
...
// Generate default samples for a preset
function generateDefaultSamples(preset: string): number[] {
  const count = 64
  const presets: Record<string, (i: number, t: number) => number> = {
  ...
  const fn = presets[preset] ?? presets.sine
  return Array.from({ length: count }, (_, i) => fn(i, count))
}

// Frequency control
const frequency = computed({
  get: () => (props.data?.frequency as number) ?? 440,
  set: (value: number) => {
    flowsStore.updateNodeData(props.id, { frequency: value })
  },
})

// Volume control
const volume = computed({
  get: () => (props.data?.volume as number) ?? 0.5,
  set: (value: number) => {
    flowsStore.updateNodeData(props.id, { volume: value })
  },
})
...
function updateWaveform(value: WaveformData) {
  waveformData.value = value
}
</script>
    ...
    <div class="node-content">
      ...
      >
        <Waves
          ...
          class="header-icon"
        />
        <span class="node-title">Wavetable</span>
        <button
      ...
      >
        ...
        >
          ...
          <input
            ...
            max="2000"
            step="1"
            @input="frequency = parseFloat(($event.target as HTMLInputElement).value) || 440"
          >
        ...
        >
          ...
          <input
            ...
            max="1"
            step="0.01"
            @input="volume = parseFloat(($event.target as HTMLInputElement).value)"
          >
...
.wavetable-node.selected .node-content {
  border-color: var(--color-primary-400);
  box-shadow: 4px 4px 0 0 var(--color-primary-200);
}

.wavetable-node:hover .node-content {
...
.wavetable-node.collapsed .handles-column {
  justify-content: center;
  padding-top: 0;
  gap: 0;
}

.wavetable-node.collapsed .handle-slot {
  position: absolute;
  top: 50%;
...
.wavetable-node.collapsed .port-label {
  display: none;
}
...
.wavetable-node.collapsed .node-header {
  border-radius: var(--radius-default);
  border-bottom: none;
}
